...
neutral <- read.delim("Andes.GuaHigh.neutralPBE.txt")
head(neutral)
neutral["ID"] <- paste(neutral$CHROM, neutral$POS, sep="_")

## find out the neutral SNP type (co, anti, no-dire)
SNPtype <- read.delim("../Andes.GuaHigh.parallel.SNPtype")
head(SNPtype)
names(SNPtype)
SNPtype.slim <- SNPtype[, c(1,14,16)]
head(SNPtype.slim)
...
neutral.SNPtype3$MexLow.ALT <- cut(neutral.SNPtype3$freqALT_MexLow, breaks=seq(0, 1, 0.1), labels=seq(0, 0.9, 0.1))
...
neutral.SNPtype3$MexLow.ALT <- ifelse(neutral.SNPtype3$freqALT_MexLow==0, 0, neutral.SNPtype3$MexLow.ALT)

head(neutral.SNPtype3)
write.table(neutral.SNPtype3[, c(1,5,6,4)], file="GH_Andes.neutralSNP.ancestral2dsfs.txt", quote=F, sep="\t", row.names=F)


# find out the 2dsfs in outlier SNPs
MH.Andes.outlier <- read.delim("../Andes.GuaHigh.outlier.SNPtype")
head(MH.Andes.outlier)
parv.freq <- fread("parv.slim.frq")
head(parv.freq)
names(parv.freq)[3:4] <- c("parv.REF", "parv.ALT")
parv.freq <- as.data.frame(parv.freq)
...
head(parv.freq)
head(MH.Andes.outlier)
length(MH.Andes.outlier$ID)
df <- merge(MH.Andes.outlier, parv.freq, by="ID")
length(df$ID)
names(df)
df.slim <- df[, c(1:3, 13:14, 19:20)]
head(df.slim)
df.slim$MexLow.ALT <- cut(df.slim$freqALT_MexLow, breaks=seq(0, 1, 0.1), labels=seq(0, 0.9, 0.1))
...
df.slim$MexLow.ALT <- ifelse(df.slim$freqALT_MexLow==0, 0, df.slim$MexLow.ALT)
head(df.slim)
...
write.table(t, file="GH.Andes.outlier.ancestral2dsfs.txt", quote=F, sep="\t", row.names=F)
...
test.list2 <- sapply(test.list, function(x)sample(x$ID, unique(x$freq)))
