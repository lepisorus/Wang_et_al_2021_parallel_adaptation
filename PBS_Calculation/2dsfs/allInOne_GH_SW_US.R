...
##read in the common neutral SNPs, the new neutral file
neutral <- read.delim("SW_US.GuaHigh.neutralPBE.txt")
head(neutral)
neutral["ID"] <- paste(neutral$CHROM, neutral$POS, sep="_")

## find out the neutral SNP type (co, anti, no-dire)
SNPtype <- read.delim("../SW_US.GuaHigh.parallel.SNPtype")
head(SNPtype)
names(SNPtype)
...
head(neutral.SNPtype3)
write.table(neutral.SNPtype3[, c(1,5,6,4)], file="SW_US_GH.neutralSNP.ancestral2dsfs.txt", quote=F, sep="\t", row.names=F)
...
write.table(t, file="SW_US_GH.outlier.ancestral2dsfs.txt", quote=F, sep="\t", row.names=F)
