...
neutral <- read.delim("MexHigh.Andes.neutralPBE.txt")
head(neutral)
neutral["ID"] <- paste(neutral$CHROM, neutral$POS, sep="_")

## find out the neutral SNP type (co, anti, no-dire)
SNPtype <- read.delim("../MexHigh.Andes.parallel.SNPtype")
head(SNPtype)
names(SNPtype)
SNPtype.slim <- SNPtype[, c(1,7,16)]
head(SNPtype.slim)
...
head(neutral.SNPtype3)
write.table(neutral.SNPtype3[, c(1,5,6,4)], file="MH_Andes.neutralSNP.ancestral2dsfs.txt", quote=F, sep="\t", row.names=F)


# find out the 2dsfs in outlier SNPs
MH.Andes.outlier <- read.delim("../MexHigh.Andes.outlier.SNPtype")
...
write.table(t, file="MH.Andes.outlier.ancestral2dsfs.txt", quote=F, sep="\t", row.names=F)
