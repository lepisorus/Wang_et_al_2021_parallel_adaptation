#! These scripts have been modified to just use a single location of the selected site for a given window
#! All new comments are marked with "#!"

#! Additionally, the script has been modified to use coancestry matrices (and their corresponding det and inv)
#!  generated with for distances in (0, 10,000]
#!  Here, we cut the distances of the positions of the sites in the window to the selected site into the same bins used to generate these matrices.
#!  Selected sites should not be included in the analysis and will not be place in a bin.

#! I also moved the randomization of the allele frequencies with respect to the reference allele and mean-centering to a different script.
#!  The idea is that we will do this step first and only once as some sites may occur in multiple windows.
...
numBins = 1000
selSite_proxy = 10001
positions_proxy = seq(1, 20001, by = 10)
distances_proxy = abs(positions_proxy - selSite_proxy)
my.seq = seq(min(distances_proxy), max(distances_proxy), length.out = (numBins + 1))
distances = abs(positions - selSite) #! This is the actual distances of sites in the window to the selected site
...
#! Previously this was a matrix (when there are multiple proposed selected sites) as generated in commented-out line above

distBins = as.numeric(cut(distances, my.seq)) #! modified for a single selected site
...
##neutral model
calcLikelihood_bin_neutral.1site = function(site, det_FOmegas, inv_FOmegas) {
...
calcCompLikelihood_neutral.1site = function(det_FOmegas, inv_FOmegas) {
  ...
  #	det_FOmegas: list of length numBins of determinants of variance/covariance matrices for
	...
	all = sapply(1 : length(positions), {
			function(i) calcLikelihood_bin_neutral.1site(i, det_FOmegas, inv_FOmegas)
		})
	return(sum(all))
}

calcLikelihood_bin_1par.1site = function(site, det_FOmegas, inv_FOmegas, par1) {
...
calcCompLikelihood_1par.1site = function(det_FOmegas, inv_FOmegas, par1) {
	...
	all = sapply(1 : length(positions), {
			function(i) calcLikelihood_bin_1par.1site(i, det_FOmegas, inv_FOmegas, par1)
	})
	return(sum(all))
}

calcLikelihood_bin_3par.1site = function(site, det_FOmegas, inv_FOmegas, par1, par2, par3) {
	# Calculates log-likelihood of data at a given position for models
...
calcCompLikelihood_3par.1site = function(det_FOmegas, inv_FOmegas, par1, par2, par3) {
	# Calculates composite log-likelihood of all data for models with a 
	...
	#	composite log-likelihood of data under model
  all = sapply(1 : length(positions), {
    function(i) calcLikelihood_bin_3par.1site(i, det_FOmegas, inv_FOmegas, par1, par2, par3)
  })
	return(sum(all))
}

calcLikelihood_bin_4par.1site = function(site, det_FOmegas, inv_FOmegas, par1, par2, par3, par4) {
	# Calculates log-likelihood of data at a given position for models
...
calcCompLikelihood_4par.1site = function(det_FOmegas, inv_FOmegas, par1, par2, par3, par4) {
	# Calculates composite log-likelihood of all data for models with a 
	...
	#	composite log-likelihood of data under model
	all = sapply(1 : length(positions), {
			function(i) calcLikelihood_bin_4par.1site(i, det_FOmegas, inv_FOmegas, par1, par2, par3, par4)
		})
	return(sum(all))
}

calcLikelihood_bin_5par.1site = function(site, det_FOmegas, inv_FOmegas, par1, par2, par3, par4, par5) {
	# Calculates log-likelihood of data at a given position for models
...
calcCompLikelihood_5par = function(det_FOmegas, inv_FOmegas, par1, par2, par3, par4, par5) {
	# Calculates composite log-likelihood of all data for models with a 
	...
	#	composite log-likelihood of data under model
	all = sapply(1 : length(positions), {
			function(i) calcLikelihood_bin_5par.1site(i, det_FOmegas, inv_FOmegas, par1, par2, par3, par4, par5)
	})
	return(sum(all))
}
